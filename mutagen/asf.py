...
from mutagen._util import (insert_bytes, delete_bytes, DictMixin,
                           total_ordering, MutagenError, get_fileobj)
...
class ASF(FileType):
    ...
    def save(self, filename=None):
        """
        Saves the file with metadata to the specified filename.
        filename can be a path or a file object opened in mode r+b.
        """
        ...
        with get_fileobj(self, filename) as fileobj:
            size = len(data)
            if size > self.size:
