...
from mutagen._util import insert_bytes, MutagenError, get_fileobj
...
class FLAC(mutagen.FileType):
    ...
    * pictures -- list of embedded pictures
    ...
    def load(self, filename):
        ...
        fileobj = StrictFileObject(get_fileobj(self, filename))
        try:
    ...
    def save(self, filename=None, deleteid3=False):
        ...
        If no filename is given, the one most recently loaded is used.
        filename must be either a file object opened in mode r+b or a path.
        """
        f = get_fileobj(self, filename)
